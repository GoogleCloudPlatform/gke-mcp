...
func installQueryLogsTool(s *server.MCPServer, conf *config.Config) error {
	queryLogsTool := mcp.NewTool("query_logs",
		...
		mcp.WithString("query", mcp.Description("LQL query string to filter and retrieve log entries. Don't specify time ranges in this filter. Use 'time_range' instead. Consider using https://cloud.google.com/logging/docs/view/logging-query-language to learn more about LQL syntax.")),
		...
		mcp.WithString("since", mcp.Description("Only return logs newer than a relative duration like 5s, 2m, or 3h")),
...
func NewQueryLogsTool(conf *config.Config) (*QueryLogsTool, error) {
	client, err := logging.NewClient(context.TODO(), option.WithUserAgent(conf.UserAgent()))
	if err != nil {
...
func (t *QueryLogsTool) queryGCPLogs(ctx context.Context, req LogQueryRequest) (string, error) {
	resp := t.client.ListLogEntries(ctx, buildListLogEntriesRequest(req))
